...
public record ViewPort(int xCoord, int yCoord, int width, int height) {
    ...
    private ViewPort centeredViewPort(int width, int height) {
        return new ViewPort(xCoordCenterFor(width), yCoordCenterFor(height), width, height);
    }
    ...
    private ViewPort fitToWidthViewPort(int width, int height) {
        return new ViewPort(
                0,
                yCoordCenterFor(calculateNewHeight(width, height)),
                this.width,
    ...
    private ViewPort fitToHeightViewPort(int width, int height) {
        return new ViewPort(
                xCoordCenterFor(calculateNewWidth(width, height)),
                0,
    ...
    private int xCoordCenterFor(int width) {
        return (this.width - width) / 2;
    }

    private int yCoordCenterFor(int height) {
